...
public class DataAccessAnnotationBeanProcessor extends InstantiationAwareBeanPostProcessorAdapter {
  ...
  class DataAccessFieldCallback implements FieldCallback {
    ...
    public void doWith(Field field) throws IllegalArgumentException, IllegalAccessException {
      ...
      String beanName =
          field.getType().getSimpleName() + BEAN_SEPARATOR + entityClz.getSimpleName();
      Preconditions.checkArgument(entityClz.isAnnotationPresent(Entity.class),
          "注入字段[%s]的类型声明必须为Entity注解标注", field.getName());
    ...
    private Object injectEntityService(String beanName, Class<? extends IEntity> entityClz) {
      // 判断是否含有Id注解的字段
      Preconditions.checkArgument(hasIdField(entityClz), "实体[%s]不存在Id注解字段",
      ...
      beanFactory.autowireBeanProperties(initializeBean,
